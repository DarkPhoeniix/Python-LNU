...
def is_dot_in_area(x_val, y_val, func_f, func_g):
    f = sp.lambdify(x, func_f, "numpy")
    g = sp.lambdify(x, func_g, "numpy")
    if (y_val >= f(x_val) and y_val <= g(x_val)) or \
        (y_val >= g(x_val) and y_val <= f(x_val)):
        return True
    return False
...
random_dots_x = [random.uniform(a, b) for i in range(5000)]
random_dots_y = [random.uniform(min(min(fy_values), min(gy_values)),
                                max(max(fy_values), max(gy_values))) for i in range(5000)]
dots_in_area_x = []
dots_in_area_y = []
to_del = []
...
plt = plot.plot(x_values,
                fy_values,
                color='#690415')
plt = plot.scatter(random_dots_x,
                   random_dots_y,
                   color='#c7c7c7',
                   s=0.5)
plt = plot.vlines(a,
...
plt = plot.vlines(b,
                  ...
                  color='#adadad',
                  linestyles='dashed')
plt = plot.hlines(max(max(fy_values), max(gy_values)),
                  xmin=min(x_values),
                  xmax=max(x_values),
                  color='#adadad',
                  linestyles='dashed')
plt = plot.hlines(min(min(fy_values), min(gy_values)),
                  xmin=min(x_values),
                  xmax=max(x_values),
                  color='#adadad',
                  linestyles='dashed')
plt = plot.scatter(dots_in_area_x,
                   dots_in_area_y,
                   color='#317d4a',
                   s=0.5)
plt = plot.plot(x_values,
