...
while True:
    ...
    exit - exit program''')
        ...
        if command_line[1] == 'USD':
            draw_plot(datetime.datetime.strptime(command_line[2], "%d/%m/%Y").date(),
                      datetime.datetime.strptime(command_line[3], "%d/%m/%Y").date(),
                      read_data_from_file('USD', r'database_USD.txt'))
        if command_line[1] == 'EUR':
            draw_plot(datetime.datetime.strptime(command_line[2], "%d/%m/%Y").date(),
                      datetime.datetime.strptime(command_line[3], "%d/%m/%Y").date(),
                      read_data_from_file('EUR', r'database_EUR.txt'))
        if command_line[1] == 'RUB':
            draw_plot(datetime.datetime.strptime(command_line[2], "%d/%m/%Y").date(),
                      datetime.datetime.strptime(command_line[3], "%d/%m/%Y").date(),
